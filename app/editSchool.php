<?php 
	include("php/doSchoolDetail.php");
...
function validateForm(form) {
    ...
    var name = form.sch_name.value;
	var sch_educaiton_level = form.sch_educaiton_level.value;
	...
	if (sch_educaiton_level == "") {
		errors.push("Please select education level");
    }
	
	if(location_postalcode==""){
		errors.push("Please enter postal code");
	}	else if(!RE_POSTALCODE.test(location_postalcode)){
...
<!-- Side bar-->
	<?php include("php/sidebar/manageSideBar.php");?>
        ...
        <div class="col-xs-12 col-sm-9">
			...
			<h1>Edit School</h1>
			<hr>
 <form action="php/doEditSchool.php" method="post" style="padding:20px" class="spanx well" onsubmit="return validateForm(this)">
    ...
    <div class="form-group">
	  <label for="sch_name" style="padding-bottom:8px;" tabindex="1" >School Name: </label>
	  <input type="text"  name="sch_name" class="form-control" value="<?php echo $sch_name;?>" />
	</div>
	
	<div class="form-group required" >
		<label for="sch_educaiton_level" style="padding-bottom:8px;" tabindex="1">Education Level: </label>
        <select name="sch_educaiton_level" id="reg_gender" class="form-control" tabindex="2" >
			<?php 
		for($i=0;$i<count($eduLevelArr) ; $i++){
			$eduLevel = $eduLevelArr[$i];
			$eduCateUpper = strtoupper($eduLevelArr[$i]);
			if(strcasecmp($sch_edu ,$eduLevel)==0){
				echo "<option value='$eduCateUpper' selected='selected'>$eduLevel</option>";	
			}
			else{
				echo "<option value='$eduCateUpper'>$eduLevel</option>";	
			}
	...
	<div class="form-group">
	  ...
	  <input type="text"  name="location_address" class="form-control"  value="<?php echo $sch_add?>" >

	</div>
	<div class="form-group">
	  <label for="location_postalcode" style="padding-bottom:8px;" tabindex="1">Postal Code: </label>
	  <input type="number" step=1 name="location_postalcode" class="form-control" value="<?php echo $sch_postal?>" >
	</div>
	
	<div class="form-group">
	  <label for="location_latitude" style="padding-bottom:8px;" tabindex="1">Latitude: </label>
	  <input type="number" step=0.00001 name="location_latitude" class="form-control" value="<?php echo $sch_lat?>"  >
	</div>
	
	<div class="form-group">
	  <label for="location_longtitude" style="padding-bottom:8px;" tabindex="1">Longtitude: </label>
	  <input type="number" step=0.0001 name="location_longtitude" class="form-control" value="<?php echo $sch_long?>"  >
	</div>	
	
    <div class="form-group">
        <div class="row">
            <div class="col-sm-6 col-sm-offset-3">
                <a  class="btn btn-large  btn-primary " style="width:40%;margin-right:10px;" href="aManage.php" >Back</a>
